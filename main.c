...
void Filter_Configuration(CAN_HandleTypeDef *hcan);
...
void HAL_CAN_RxFifo0MsgPendingCallback(CAN_HandleTypeDef *hcan)
{
  ...
  if(HAL_CAN_GetRxMessage(hcan, CAN_RX_FIFO0, &RxHeader, &percentage) != HAL_OK)
  {
	  Error_Handler();
  }
  ...
  char msg[50];
  sprintf(msg,"Valoarea procentului este: %d\r\n",percentage);
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim)
{
    ...
    uint8_t our_message[5] = {'P','E','P','S','I'};


  	CAN_TxHeaderTypeDef TxHeader;


  	TxHeader.DLC = strlen(our_message);
